...
Measured on an idle tunnel, single track, 30s window ([`idle_rate_test.go`](relay/tunnel/idle_rate_test.go)):

| | packets/s | CPU (one core) |
|---|---|---|
| upstream | 7.9 | 2.155 % |
| this build | **0.2** | **0.007 %** |

Idle traffic drops ~40x and CPU ~300x. Two fixed-rate tickers no longer spin with nothing to send — the VP8 writer (648 wakeups/s) and the KCP update loop (100/s) — and the keepalive period, previously nailed to 60–200ms, is now configurable and defaults to 3–8s.
